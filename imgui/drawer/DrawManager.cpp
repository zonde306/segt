...
void DrawManager::CreateObjects()
{
	...
	if(FAILED(D3DXCreateFontA(m_pDevice, 12, 0, FW_NORMAL, 0, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH, "Arial", &m_pDefaultFont)))
	{
...
void DrawManager::RenderCircle(unsigned long color, int x, int y, int r, int resolution)
{
	...
	{
		curPointX = (float)(x + r * cos(2 * PI * i / resolution));
		curPointY = (float)(y - r * sin(2 * PI * i / resolution));
		if(i > 0)
		{
